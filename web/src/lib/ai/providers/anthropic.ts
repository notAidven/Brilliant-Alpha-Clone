 ...
 * and the raw-key read below is dead-code-eliminated so a `VITE_ANTHROPIC_API_KEY` can
...
function apiKey(): string {
  ...
  // `VITE_ANTHROPIC_API_KEY` value is never inlined into (and shipped with) the bundle.
...
async function generateText(prompt: string, signal: AbortSignal): Promise<string | null> {
  ...
  // A raw `VITE_ANTHROPIC_API_KEY` would be exposed to every visitor; the secure
  ...
  if (import.meta.env.PROD) {
    throw new Error(
      "The 'anthropic' direct-key provider is disabled in production builds to prevent " +
